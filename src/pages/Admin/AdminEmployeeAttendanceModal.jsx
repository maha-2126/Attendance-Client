...
const AdminEmployeeAttendanceModal = ({ employee, onClose }) => {
    ...
    const handleDownload = (employeeName, selectedDate, summary, records) => {
        ...
        // 2. Calendar Details CSV
        const calendarCsv = [
            ['Date', 'Status'],
            ...records.map(({ date, status }) => [date, status])
        ]
            .map((row) => row.join(','))
            .join('\n');

        // 3. Combine both sections
        const combinedCsv = `Attendance Summary for ${employeeName} - ${monthYear}\n\n${summaryCsv}\n\nDetailed Records\n${calendarCsv}`;
        ...
        link.href = url;
        link.download = `Attendance_Summary_${employeeName}_${monthYear.replace(' ', '_')}.csv`;
