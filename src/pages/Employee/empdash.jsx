  ...
  const fetchDateStatusList = async (employeeId, year, month) => {
    try {
      ...
      if (Array.isArray(data.dailyRecords)) {
        setDateStatusList(data.dailyRecords);
      } else {
        setDateStatusList([]);
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
       ...
       <div className="bg-black/30 p-4 rounded-xl shadow-inner">
        ...
        {checkOutTime && checkOutTime !== '-' ? (
          <p className="text-sm text-green-400 mt-1">✅ Check-Out Time: {checkOutTime}</p>
        ) : (
          <p className="text-sm text-red-400 mt-1">❌ Not Checked Out</p>
        )}
      </div>
